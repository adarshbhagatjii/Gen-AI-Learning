...
if st.button("Process"):
    ...
    for word in words:
        ...
        if stem.lower() != spacy_lemma.lower():
            highlight = '<span style="color:green;">(Changed)</span>'
            st.markdown(f"**{word}** -> ({stem}) != **{spacy_lemma}** {highlight}", unsafe_allow_html=True)
        elif word.lower!=spacy_lemma.lower():
            highlight = '<span style="color:red;">(Changed)</span>'
            st.markdown(f"**{word}** ({word}) != **{spacy_lemma}** {highlight}", unsafe_allow_html=True)
